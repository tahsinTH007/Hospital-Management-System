...
/**
 * The job bodies only use `step.run`, so they can also execute inline with a
 * pass-through step (see dispatch.ts) when Inngest is not available.
 */
export type JobStep = Pick<GetStepTools<typeof inngest>, "run">;
...
export interface AnalyzeXRayData {
  labResultId: string;
  imageUrl: string;
  bodyPart?: string;
}
...
export const runAdmitPatient = async (
  { patientId, admissionReason }: AdmitPatientData,
  step: JobStep,
) => {
  await connectDB();
  ...
  const aiAssignment = await step.run("ai-triage", async () => {
    ...
    const doctor = data.doctors.find(
      (d) => d._id.toString() === parsed.doctorId,
    );
...
export const admitPatient = inngest.createFunction(
  ...
  ({ event, step }) => runAdmitPatient(event.data as AdmitPatientData, step),
);

export const runAnalyzeXRay = async (
  { labResultId, imageUrl, bodyPart }: AnalyzeXRayData,
  step: JobStep,
) => {
  await connectDB();
...
export const analyzeXRayJob = inngest.createFunction(
  ...
  ({ event, step }) => runAnalyzeXRay(event.data as AnalyzeXRayData, step),
);

export const runAddCharge = async (
  { patientId, description, priceInCents }: AddChargeData,
  step: JobStep,
) => {
...
export const addChargeToInvoice = inngest.createFunction(
  ...
  ({ event, step }) => runAddCharge(event.data as AddChargeData, step),
);
